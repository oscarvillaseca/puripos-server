...
function renderPeriodo(d){
  ...
  const gastos = d.gastos_mes||0;
  const neto = d.neto - costo - gastos;
  const netoColor = neto>=0?'#10b981':'#ef4444';
  let notaGastos = '';
  if(gastos>0){
    if(PERIODO==='hoy') notaGastos='* Estimado diario de gastos mensuales';
    else if(PERIODO==='semana') notaGastos='* Estimado semanal de gastos mensuales';
  }
  const resumenHTML=`
