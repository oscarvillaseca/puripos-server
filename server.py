...
function render(d){
  ...
  document.getElementById('app').innerHTML=`
    ...
    ${(function(){
      if(!d.inventario||d.inventario.length===0) return '';
      var rows = d.inventario.map(function(p){
        var sym = p.critico ? '&#10060;' : p.bajo ? '&#9888;' : '&#10003;';
        var col = p.critico ? '#ef4444' : p.bajo ? '#f59e0b' : '#10b981';
        var mn  = p.minimo>0 ? '<span style="color:#64748b;font-size:11px"> min:'+p.minimo+p.unidad+'</span>' : '';
        return '<div class="stock-item">'
          +'<div><span class="stock-sym" style="color:'+col+'">'+sym+'</span> '+p.nombre+'</div>'
          +'<div style="text-align:right"><span style="color:'+col+';font-weight:500">'+p.stock+' '+p.unidad+'</span>'+mn+'</div>'
          +'</div>';
      }).join('');
      return '<div class="seccion"><div class="sec-title">stock bodega</div>'+rows+'</div>';
    })()}
