...
function renderPeriodo(d){
  ...
  const const inicio = d.turno_inicio && d.turno_inicio!=='—' ? ` · Inicio: ${d.turno_inicio}` : '';
  turnoHTML=`<div class="turno">
    <div class="avatar">${ini}</div>
    <div>
      <div class="turno-cajero">${d.cajero||'—'} · Turno activo</div>
      <div class="turno-sub">${d.n_pedidos||0} ventas · ${fmt(d.neto)}${inicio}</div>
    </div>
  </div>`;
